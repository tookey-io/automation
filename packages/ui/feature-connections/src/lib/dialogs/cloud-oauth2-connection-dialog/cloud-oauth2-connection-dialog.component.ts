...
import { AppConnectionsService, environment, fadeInUp400ms } from '@activepieces/ui/common';
...
export class CloudOAuth2ConnectionDialogComponent implements OnInit {
    ...
    constructor(
        ...
        this._cloudConnectionPopupSettings = {
            ...
            redirect_url: environment.secretsUrl + '/redirect',
    ...
    createPropsFormGroup() {
        ...
        if (this.dialogData.pieceAuthProperty.props) {
            Object.keys(this.dialogData.pieceAuthProperty.props).forEach(
                (key) => {
    ...
    get authenticationSettingsControlsValid() {
        return Object.keys(this.settingsForm.controls)
            .filter(
                (key) =>
                    key !== 'connection' &&
                    !this.settingsForm.controls[key as keyof AuthConfigSettings]
                        .disabled
            )
            .map((key) => {
                return this.settingsForm.controls[
                    key as keyof AuthConfigSettings
                ].valid;
            })
            .reduce((prev, next) => {
                return prev && next;
